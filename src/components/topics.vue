  ...
  <b-container fluid>
    <b-row>
      <div class="h3">
        ...
        <b-badge :variant="activeQueryTopics.find(a => a.title == t.title)?  'primary':'secondary' "
                 @click="selectTopic(t.title,$event)" v-for=" t in topicList" v-bind:key="t.title">{{ t.title }}
        </b-badge>
      </div>
    </b-row>
    <b-row>
...
export default {
  ...
  data() {
    return {
      ...
      topicList: [{
        title: 'Variant', query: '(variants||mutations||deletions )'
      },
        {
          title: 'Surveillance', query: '(wasterwater|| surveillance)'
        },
        {
          title: 'Diagnostics', query: '(diagnostics||tests||rt-PCR||qPCR||rt-qPCR)'
        },
      ],
      dateSelectors: [
  ...
  computed: {
    topicsAsText() {
      if (this.activeQueryTopics.length === 0) {
        return this.topicList.map(t => t.title).join(' OR ')
      } else {
        return this.activeQueryTopics.map(t => t.title).join(' AND ')
      }
    },
  },
  methods: {
